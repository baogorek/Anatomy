// Bilateral, unregistered neck reference meshes, extracted without fitting.
...
try {
  ...
  if (data.missingNames.length)
    throw Error(
      `Missing or duplicate neck surfaces: ${data.missingNames.join(", ")}`,
    );
  writeFileSync(
    "public/models/muscle-reference/neck.json",
    JSON.stringify({
      schema: 1,
      dataset: "neck",
      ...data,
  ...
  console.log(
    `Neck atlas: ${data.meshes.filter((m) => !m.bone).length} bilateral muscle surfaces; ${data.meshes.filter((m) => m.bone).length} source bone meshes.`,
  );
} finally {
  await browser.close();
}
